...
│  └──────────────┘  └──────────────┘  └──────────────┘      │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐      │
│  │ Analytics    │  │ Price History│  │ Decision     │      │
│  └──────────────┘  └──────────────┘  └──────────────┘      │
│  ┌──────────────┐  ┌──────────────┐                        │
│  │ Admin Stats  │  │ Auth Service │                        │
│  └──────────────┘  └──────────────┘                        │
└─────────────────────────────────────────────────────────────┘
                            │
                            ▼
...
4. **配置管理**: 环境变量管理、交互式配置
5. **认证授权**: 管理员登录验证、用户 JWT 认证
6. **数据同步**: 多设备数据同步（购物车、会话）
7. **数据分析**: 消费结构分析、偏好分析、时间分析（新增）
8. **价格历史**: 价格历史记录和趋势分析（新增）
9. **购物决策**: 商品对比、评分计算、替代推荐（新增）
10. **管理员后台**: 用户统计、系统监控、搜索热词（新增）
11. **调试支持**: 调试端点、日志记录
...
**ConsumptionStructureAnalyzer**:
- 计算消费结构（品类分布、价格区间、平台偏好）
- 生成图表数据
...
| 端点 | 方法 | 功能 | 认证 |
|------|------|------|------|
| `/api/v1/analytics/consumption-structure` | GET | 获取消费结构分析 | 可选 |
| `/api/v1/analytics/preferences` | GET | 获取用户偏好分析 | 可选 |
| `/api/v1/analytics/shopping-time` | GET | 获取购物时间分析 | 可选 |
...
| `/api/v1/price-history/record` | POST | 记录价格历史 | 可选 |
...
| 端点 | 方法 | 功能 | 认证 |
|------|------|------|------|
| `/api/v1/decision/compare` | POST | 对比多个商品 | 否 |
| `/api/v1/decision/score` | POST | 计算购买建议评分 | 否 |
...
- 分析搜索热词
- 从消息表的 keywords 字段聚合统计

#### 4.13.3 管理员后台 API 端点

| 端点 | 方法 | 功能 | 认证 |
|------|------|------|------|
| `/api/v1/admin/users/stats` | GET | 获取用户统计数据 | 管理员 |
| `/api/v1/admin/system/stats` | GET | 获取系统使用情况 | 管理员 |
| `/api/v1/admin/search/keywords` | GET | 获取搜索热词统计 | 管理员 |
| `/api/v1/admin/export` | GET | 导出统计数据 | 管理员 |
...
| 1.0 | 2026 | 初始后端架构文档 | CHYINAN (Architect) |
